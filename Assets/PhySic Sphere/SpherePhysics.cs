...
public class SpherePhysics : MonoBehaviour
{
	...
	private Sphere[] tmpSphereList = new Sphere[1];
	...
	void Update()
	{
		...
		SphereBuffer.GetData(tmpSphereList);

		Debug.Log(tmpSphereList[0].velocity);
	}
	...
	void UpdateBuffers()
	{
		...
		for (int i = 0; i < instanceCount; i++)
		{
			...
			entities[i].position.x = Random.Range(-3.0f,3.0f);
			entities[i].position.y = 20.0f;
			entities[i].position.z = Random.Range(-3.0f, 3.0f);
			//Debug.Log(entities[i].position);
			entities[i].velocity = Vector3.zero;
