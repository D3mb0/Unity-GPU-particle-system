...
Shader "Draw Sphere"
{
	Subshader
	{
		Pass
		{
			...
			#pragma target 5.0
			float4 _initPosition;
			float4 _spherColor;


			float4 VSMain(uint id:SV_VertexID, out float3 p : TEXCOORD0, out float3 finalcolor : TEXCOORD1) : SV_POSITION
			{
				float f = id;
				...
				finalcolor = _spherColor.xyz;
				p += _initPosition.xyz;
				return UnityObjectToClipPos(float4(p, 1.0));
			}

			float4 PSMain(float4 s:SV_POSITION, float3 p : TEXCOORD0, float3 finalcolor : TEXCOORD1) : SV_Target
			{
				...
				float4 tmp = float4((diffuse1 + diffuse2+ diffuse3),1.0);
				return tmp;
			}
			ENDCG
		}
