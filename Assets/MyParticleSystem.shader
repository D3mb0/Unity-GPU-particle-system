	...
	Subshader
	{
		Pass
		{
			...
			float4 VSMain(uint id:SV_VertexID, out float3 normal : TEXCOORD0, out float3 position : TEXCOORD1, out float instance : TEXCOORD2, out float3 color : TEXCOORD3, out float3 finalcolor : TEXCOORD4) : SV_POSITION
			{
				...
				position = mul(rotationY(_Time.g * 1.7 - instance), mul(rotationX(_Time.g * 3.0 + instance),position)); // for rotatem cube
				color = position;
				...
				finalcolor = float3(1, 1, 1); //change color cube
				float range3 = remap(_Spread * hash(uint(instance)),0.0,_Spread,-_Spread,_Spread);
				//position += Parabola(uv, uv * range3.xxx, _Height, fmod(_Timer * _Speed + 2.0 * (_Lifetime + factor),_Lifetime + factor)); // for update cube
				position += Parabola(uv, uv * range3.xxx, _Height, fmod(_Timer * _Speed + 2.0 * (_Lifetime + factor), _Lifetime + factor)); // for update cube
				if (range >= float(_EmitterCycles)) return 0;
				position *= float3(_ParticleScale,_ParticleScale,_ParticleScale); // for scaling particle
